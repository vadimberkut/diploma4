...
const int MeanShiftParams::SPATIAL_RADIUS_DEFAULT = 10;
const double MeanShiftParams::COLOR_RADIUS_DEFAULT = 6.5;//6.5;
const int MeanShiftParams::MIN_REGION_SIZE_DEFAULT = 50;

int MeanShiftParams::SPATIAL_RADIUS = MeanShiftParams::SPATIAL_RADIUS_DEFAULT;
double MeanShiftParams::COLOR_RADIUS = MeanShiftParams::COLOR_RADIUS_DEFAULT;//6.5;
int MeanShiftParams::MIN_REGION_SIZE = MeanShiftParams::MIN_REGION_SIZE_DEFAULT;
...
int MeanShift(const IplImage* img, int **labels)
{
	...
	int level = 1;
	double COLOR_RADIUS2 = MeanShiftParams::COLOR_RADIUS*MeanShiftParams::COLOR_RADIUS;
	int minRegion = MeanShiftParams::MIN_REGION_SIZE;//50
	...
	for(int i=0;i<img->height;i++) 
		for(int j=0;j<img->width;j++)
		{
			...
			for (int iters=0;shift > 3 && iters < 100;iters++) 
			{
				...
				int i2from = cv::max(0, i - MeanShiftParams::SPATIAL_RADIUS), i2to = cv::min(img->height, i + MeanShiftParams::SPATIAL_RADIUS + 1);
				int j2from = cv::max(0, j - MeanShiftParams::SPATIAL_RADIUS), j2to = cv::min(img->width, j + MeanShiftParams::SPATIAL_RADIUS + 1);
